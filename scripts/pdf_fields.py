...
def fill_pdf(template_path: Path, output_path: Path, flat_data: dict):
    """Fill the PDF form using fillpdfs."""
...
def main():
    ...
    default_tpl = Path("/home/abdulwahab-bhai/personal/tradia_ai_backend/backend/docs/b650_unlocked.pdf")
    default_data = Path("/home/abdulwahab-bhai/personal/tradia_ai_backend/backend/forms_schema/b650_llm_response.json")
    default_map = Path("/home/abdulwahab-bhai/personal/tradia_ai_backend/backend/forms_schema/b650_llm_to_pdf_field_map.json")
    default_out = Path("/home/abdulwahab-bhai/personal/tradia_ai_backend/backend/docs/b650_filled.pdf")

    args = sys.argv[1:]
    if len(args) == 4:
        tpl_path, data_path, map_path, out_path = map(Path, args)
    elif not args:
        tpl_path, data_path, map_path, out_path = default_tpl, default_data, default_map, default_out
    ...
    print("\nFilling PDF…")
    try:
        fill_pdf(tpl_path, out_path, flat_data)
    except Exception as e:
